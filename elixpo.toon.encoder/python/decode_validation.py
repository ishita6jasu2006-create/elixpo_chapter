...
from constants import COLON, LIST_ITEM_PREFIX
...
def validate_no_blank_lines_in_range(
    ...
    first_blank = next(
        (blank for blank in blank_lines if start_line < blank.line_number < end_line),
        None
    )
    if first_blank:
        raise SyntaxError(
...
def is_data_row(content: str, delimiter: str) -> bool:
